...
const queries = require('./homekit.queries.js');
...
function getIdModule() {
  return gladys.module.get()
    .then(modules => Promise.filter(modules, module => module.slug === slug)
      .then(filteredModule => filteredModule[0].id)
      .catch(() => Promise.reject(new Error('id not found !'))));
}
